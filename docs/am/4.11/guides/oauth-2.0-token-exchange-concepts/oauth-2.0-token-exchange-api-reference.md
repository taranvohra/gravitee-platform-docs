...
When delegation is used, the issued access token includes an `act` claim identifying the actor. For details on the `act` claim structure, chained delegation, and `actor_act`, see [Delegation](../developer-contributions/using-oauth-2.0-token-exchange.md#delegation).
