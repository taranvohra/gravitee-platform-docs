...
1. **Purpose:** Set a default expiration duration for all client secrets within a domain to enforce regular rotation.
...
As a workaround, to give the client secrets of an application a different lifetime than the domain default, configure an application-level expiration policy as described in [Configurable Client Secret Expiration](client-secrets.md#configurable-client-secret-expiration). To end the validity of a specific client secret before its calculated expiration date, renew or delete that secret.
