...
<table><thead><tr><th width="300">Application</th><th>Preselect consent for all scopes</th></tr></thead><tbody><tr><td>Existed before the upgrade to 4.13.0</td><td>On, so the consent page behaves as it did before the upgrade</td></tr><tr><td>Created in AM Console or through the AM Management API</td><td>Off</td></tr><tr><td>Registered through Dynamic Client Registration</td><td>On</td></tr></tbody></table>
...
{% hint style="info" %}
Both settings are also available through the [AM Management API](../../reference/am-api-reference/) application operations: `optInScopeSelection` on the application's OAuth settings, and `requiredScope` on each entry of the `scopeSettings` list.
{% endhint %}
...
{% hint style="info" %}
Revoking consent can also be done via the [AM Management API](../../reference/am-api-reference/).
{% endhint %}
