---
description: >-
  Start a hybrid Gamma Gateway container by container with the Docker CLI,
  creating the network and Redis first. Follow the steps to run it.
---
...
1. [#create-the-network](docker-cli.md#create-the-network "mention")
2. [#run-redis](docker-cli.md#run-redis "mention")
3. [#run-the-gateway](docker-cli.md#run-the-gateway "mention")
...
1.  Run the Gateway with Redis rate limiting enabled, and connect it to your Gravitee Cloud control plane:
    ...
    * Replace `<license-key>` with your License Key from Gravitee Cloud.
    ...
    <div data-gb-custom-block data-tag="hint" data-style="info" class="hint hint-info"><p>The tag sets the Gateway version. We recommend running the same version as your Gamma control plane for compatibility. You can find your control plane version in the Gravitee Cloud dashboard.</p></div>

## Verification

To confirm that your hybrid installation works, complete the following checks:

1. [#check-the-container-status](docker-cli.md#check-the-container-status "mention")
2. [#ensure-the-gateway-registers-in-gravitee-cloud](docker-cli.md#ensure-the-gateway-registers-in-gravitee-cloud "mention")
3. [#ensure-the-gateway-is-listening-locally](docker-cli.md#ensure-the-gateway-is-listening-locally "mention")
...
2.  Verify that the command output is similar to the following response:
    ...
    <div data-gb-custom-block data-tag="hint" data-style="info" class="hint hint-info"><p>This response confirms that the Gateway has initialized, but no APIs are deployed yet. Once you publish APIs through the Gamma control plane, this message is replaced by valid responses routed through the configured context paths.</p></div>
