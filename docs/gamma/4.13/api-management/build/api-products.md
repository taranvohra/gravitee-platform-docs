---
description: >-
  API Products bundle multiple API proxies into one consumer-facing product with
  shared plans. Learn when to use them and how to create one.
---
...
{% hint style="info" %}
API Products are an enterprise feature. Your organization's license must include the `apim-api-products` feature to create, update, or deploy a product. See [Configure API products](configure-your-api-product/#licensing).
{% endhint %}
...
A product's Gateway sync state is not a column on this list. It appears on the product detail page. See [Configure API products](configure-your-api-product/#gateway-sync-state).
...
* [Configure API products](configure-your-api-product/). Review licensing, plan restrictions, and the product detail page.
