---
description: >-
  Every API product property, the flag that decides which APIs can be bundled,
  and the plan types a product supports. Browse the full reference.
---
...
3. Turn on the **Allow in API Products** switch. Its helper text reads _When enabled, this API can be bundled into API Products for grouped consumer access._
...
The product's **Add API** panel only lists APIs whose `allowedInApiProducts` flag is `true`, and it hides APIs already attached to the product. The panel states this itself: _API visibility — Cannot see all your APIs? APIs must have API products enabled before they appear in this list._ It lists nothing until you type a name or context path.
...
* [Configure API products](./). Review where each configuration area lives in the product sidebar.
