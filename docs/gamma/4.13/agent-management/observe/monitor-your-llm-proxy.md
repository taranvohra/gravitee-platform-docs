---
description: >-
  Monitor token usage, cost, and model and provider mix for your LLM Proxies on
  the LLM Overview dashboard. Learn what each widget and metric shows.
---
...
Prices are configured per million tokens on the AI model in the Catalog, and a negotiated price you record there replaces the provider's suggested price wherever cost is computed. See [Add an AI model](../import/add-an-ai-model.md).

Both prices must be set for cost to be published. A price of `0` is valid and means free. An absent price means _unknown_, which is not the same thing, and no cost is recorded for that call.
...
| **Top 5 LLM Proxies**             | The most-used LLM Proxies by request count. On the environment-wide view this is your first cut at _who is generating the traffic_.                          |
...
| **Top 5 Providers by Requests**   | Request share per provider, with the total in the centre.                                                                                                    |
...
| **Top 5 Models by Tokens**  | Total token share per model, with the total in the centre.                                                                                                                                                      |
...
| **Top 5 Providers by Cost** | Total cost share per provider, with a per-model breakdown on hover and the total cost in the centre.                                                                                                            |
...
**A model shows requests and tokens, but no cost.** Cost needs both an input and an output price on the model. Check the model in the Catalog under **AI Models**: if only one price is set, or neither is, no cost is recorded for that model's traffic. Set both — use `0` for a model you are genuinely not billed for. After repricing a model, republish any LLM Proxy that consumes it so cost tracking picks up the new rate.
...
* [Add the Cost Rate Limit policy](../build/add-the-cost-rate-limit-policy.md). Cap spend on the proxies and plans the cost widgets single out.
* [Configure LLM Proxy notifications](../build/configure-llm-proxy-notifications.md). Get told about a change instead of finding it on the dashboard.
