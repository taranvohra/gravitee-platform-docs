---
description: >-
  Create an MCP Proxy that governs an upstream MCP server with authentication,
  authorization, and observability. Follow the steps in the wizard.
---
...
For Studio mode, see [Create an MCP Studio](create-an-mcp-studio/).
{% endtab %}
{% endtabs %}
...
1. [Open the MCP Proxy wizard](create-an-mcp-proxy.md#open-the-mcp-proxy-wizard)
2. [Define your proxy](create-an-mcp-proxy.md#define-your-proxy)
3. [Configure consumer security](create-an-mcp-proxy.md#configure-consumer-security)
4. [Connect to the upstream MCP server](create-an-mcp-proxy.md#connect-to-the-upstream-mcp-server)
5. [Review and create](create-an-mcp-proxy.md#review-and-create)
...
* **Configure upstream authentication**. Manage the credentials the Gateway injects when it calls the upstream server. See [Configure your MCP proxy](configure-your-mcp/).
* **Backing API**. Gravitee backs each MCP Proxy with an API in API Management. Deleting the MCP Proxy also deletes that API.

## Next steps

* [Configure your MCP proxy](configure-your-mcp/). Configure upstream authentication and other advanced settings.
* [Add policies to your MCP server](configure-your-mcp/add-policies-to-mcp-server.md). Apply fine-grained authorization.
* [Create an MCP Studio](create-an-mcp-studio/). Compose tools from this server with tools from other sources.
