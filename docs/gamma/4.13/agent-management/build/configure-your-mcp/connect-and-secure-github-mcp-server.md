---
description: >-
  Connect GitHub's MCP server to Gravitee, curate the tools it exposes, and
  secure them with authentication, fine-grained authorization, rate limits, and
  PII redaction. Follow the steps to get started.
---
...
1. [Connect the GitHub MCP server](connect-and-secure-github-mcp-server.md#connect-the-github-mcp-server)
2. [Expose the GitHub tools as a Composite MCP Server](connect-and-secure-github-mcp-server.md#expose-the-github-tools-as-a-composite-mcp-server)
3. [Authenticate access to your deployed GitHub MCP server](connect-and-secure-github-mcp-server.md#authenticate-access-to-your-deployed-github-mcp-server)
4. [Restrict GitHub tool access with fine-grained authorization](connect-and-secure-github-mcp-server.md#restrict-github-tool-access-with-fine-grained-authorization)
5. [Apply policies to the GitHub MCP server](connect-and-secure-github-mcp-server.md#apply-policies-to-the-github-mcp-server)
6. [Observe GitHub MCP interactions](connect-and-secure-github-mcp-server.md#observe-github-mcp-interactions)
...
{% hint style="info" %}
GitHub's MCP server exposes a large tool surface, and the exact set depends on the token's scopes and the account's plan. Registering the server catalogs everything discovered. The [Expose the GitHub tools as a Composite MCP Server](connect-and-secure-github-mcp-server.md#expose-the-github-tools-as-a-composite-mcp-server) section narrows that surface to the tools your agents need.
{% endhint %}
...
{% hint style="info" %}
If you compose tools from more than one upstream server and two tool names collide, assign an alias in the **Compose** step. For more information about assigning an alias, see [edit-mcp-studio-composition.md](../create-an-mcp-studio/edit-mcp-studio-composition.md "mention").
{% endhint %}

### Authenticate access to your deployed GitHub MCP server

{% hint style="info" %}
If you authenticated your GitHub MCP Server during the [Expose the GitHub tools as a Composite MCP Server](connect-and-secure-github-mcp-server.md#expose-the-github-tools-as-a-composite-mcp-server) section, you can skip this section and go to [Restrict GitHub tool access with fine-grained authorization](connect-and-secure-github-mcp-server.md#restrict-github-tool-access-with-fine-grained-authorization).
{% endhint %}
...
![The forbid statement on create\_or\_update\_file in the visual policy editor, showing the Engineering principal, the GitHub tool resource, and a Deployed status](../../../.gitbook/assets/gamma-mcp-github-forbid-policy.png)
...
Every entity reference is an identifier, not a display name. `principal` takes the group's identifier, and `resource` takes the server-qualified tool identifier. Leave `action` unconstrained unless you intend to match one specific tool invocation. A clause that references a name where an identifier is expected matches nothing, and the call is then denied by default rather than reported as a malformed policy. For more information about layered governance for MCP tools, see [govern-mcp-tool-access.md](govern-mcp-tool-access.md "mention").
{% endhint %}
...
4. Configure the limit. Set a **Limit** and a **Period**, for example 10 requests per 60 seconds, and set **Key** to an expression that resolves the caller's identity. This ensures that each identity draws on its own allowance rather than on a shared plan counter. For more information about layered governance for MCP tools, see [govern-mcp-tool-access.md](govern-mcp-tool-access.md "mention").
...
For the OpenTelemetry span view, which records the agent identity, the inputs and outputs, and the lineage of a single agent request across every tool call it made, see [inspect-your-agent-log.md](../../observe/inspect-your-agent-log.md "mention").
