---
description: >-
  Choose which gateway instances load an MCP Proxy by assigning sharding tags.
  Follow the steps on the Deployment Configuration page.
---

# Configure MCP Proxy deployment

The **Deployment Configuration** page controls where this MCP Proxy is deployed on the gateway mesh. Only gateway instances advertising matching sharding tags load its API definition.
...
{% hint style="info" %}
When the MCP Proxy is managed by the Gravitee Kubernetes Operator (GKO), the card shows **This API is managed by the Kubernetes operator. Sharding tags can only be changed from its source definition.**
{% endhint %}
