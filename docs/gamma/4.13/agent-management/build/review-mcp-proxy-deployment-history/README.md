---
description: >-
  Review every deployment of an MCP Proxy, inspect or compare versions, and roll
  back to an earlier API definition.
---

# Review MCP Proxy deployment history

The **Deployment History** page lists all deployments of this MCP Proxy, newest first. You can inspect a single version, compare two versions, and roll back to an earlier one.
...
{% hint style="warning" %}
The rollback doesn't deploy the MCP Proxy. The restored definition is saved, and the MCP Proxy is left with undeployed changes until you deploy it. The confirmation dialog describes the action as redeploying to the gateway, but no deployment takes place. The action can't be undone.
{% endhint %}

If the rollback fails, a card stays on screen reading **Rollback failed**, followed by the reason.

The rollback controls are hidden unless you have the **API\_DEFINITION** permission with the **UPDATE** access level.
