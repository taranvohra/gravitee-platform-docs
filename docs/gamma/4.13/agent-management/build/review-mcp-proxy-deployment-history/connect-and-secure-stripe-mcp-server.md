---
description: >-
  Connect Stripe's MCP server to Gravitee, withhold the tool that performs every
  write, and deny refunds per caller. Follow the steps to get started.
---
...
* **Use this when.** You want an agent to retrieve customers, charges, and invoices through Gravitee rather than connecting to Stripe's MCP server directly. You need the ability to move money governed apart from the ability to read.
* **Not covered here.**
  * [What the Stripe MCP server does and which tools it exposes](https://docs.stripe.com/mcp)
  * [govern-mcp-tool-access.md](../configure-your-mcp/govern-mcp-tool-access.md "mention")
  * [create-an-agent-identity.md](../create-an-agent-identity.md "mention")

Stripe's MCP server exposes a tool that its own definition describes as writing data through any Stripe API `POST`, `PATCH`, `PUT`, or `DELETE` operation. Creating a customer and canceling a subscription are therefore the same tool. A restricted API key limits what that tool can execute, and it does not change what the agent sees. A restricted key and an unrestricted key return the same tool list, so the model plans against the full write surface regardless of what the key can do. The granularity you need does not exist at the key layer or at the tool layer. It exists at the caller layer, which is where Gravitee applies it.
...
* An identity provider that is configured in Gravitee and holds the groups that your authorization policies reference. For more information about configuring an identity provider, see [configure-your-access-management-instance.md](../configure-your-access-management-instance.md "mention").
* If you are adding PII redaction on your environment, an AI Model Token Classification resource. For more information about AI resources, see [ai-resources.md](../ai-resources.md "mention").
...
Until you complete the authorization phase, any consumer holding a valid token for the server can invoke every tool you composed into it. That includes `create_refund`, which returns money to a customer. Complete the authorization and policy phases before you publish the server to consumers.
{% endhint %}
...
1. [Connect the Stripe MCP server](connect-and-secure-stripe-mcp-server.md#connect-the-stripe-mcp-server)
2. [Expose the Stripe tools as a Composite MCP Server](connect-and-secure-stripe-mcp-server.md#expose-the-stripe-tools-as-a-composite-mcp-server)
3. [Authenticate access to your deployed Stripe MCP server](connect-and-secure-stripe-mcp-server.md#authenticate-access-to-your-deployed-stripe-mcp-server)
4. [Restrict Stripe tool access with fine-grained authorization](connect-and-secure-stripe-mcp-server.md#restrict-stripe-tool-access-with-fine-grained-authorization)
5. [Apply policies to the Stripe MCP server](connect-and-secure-stripe-mcp-server.md#apply-policies-to-the-stripe-mcp-server)
6. [Observe Stripe MCP interactions](connect-and-secure-stripe-mcp-server.md#observe-stripe-mcp-interactions)
...
7. On the **Configure connection** step, select **Static credentials**, and then set the credential type to **Bearer token**. Enter the restricted API key. Discovery uses the key to read the server's capabilities. It is stored on the Catalog entry and is not used for runtime traffic. Runtime traffic authenticates with the credential you set on the Composite MCP Server.
...
Rather than proxying Stripe's whole tool surface, use MCP Studio to compose a Composite MCP Server that exposes only the tools a role needs. Curation is the control that precedes all the others. A tool you never compose is a tool no policy has to defend against, and one an agent's model never sees in `tools/list`.
...
`stripe_api_write` is excluded entirely, so the tool that performs every write in the Stripe API never appears in `tools/list`. The model behind the agent never learns that it exists. Curation removes what nobody should reach.
...
2.  You receive a response like the following:
    ...
    ```
3. Confirm that `stripe_api_write` is absent from the advertised scopes. Its absence shows at the protocol level, before any policy runs, that the tool is not merely denied but not present.
...
Every entity reference is an identifier, not a display name. `principal` takes the group's identifier, and `resource` takes the server-qualified tool identifier from the registered Catalog entry. For more information about layered governance for MCP tools, see [govern-mcp-tool-access.md](../configure-your-mcp/govern-mcp-tool-access.md "mention").
{% endhint %}
...
Step 4 is the result worth keeping. The error comes from Stripe, which proves that the call reached it and that the key really can refund. The identical call from the support group never left the Gateway. A payment intent that does not exist demonstrates this without creating a refund.
...
Stripe's write tools accept an optional `human_confirmation` argument that asks a person to approve the call. It is the agent that chooses whether to invoke it. An agent talked into a write is exactly the agent that does not ask first. A Gateway denial does not consult the agent.
{% endhint %}

### Apply policies to the Stripe MCP server

Authorization decides which tools a caller reaches, not how often it calls them or what comes back. An agent looping over reads can exhaust a shared Stripe allowance while staying inside its permissions. A permitted read returns customer names, email addresses, and phone numbers by construction. Add a rate limit and a redaction policy on the `tools/call` flow to close both.
...
6. Select **Save**, and then deploy the server. The deploy is what updates the Gateway. Until you deploy, the console reports that the deployable is out of sync.

{% hint style="info" %}
To limit one tool rather than the whole server, add a **Condition** to the flow that matches the tool name, for example `{#context.attributes['mcp_tool_name'] == 'stripe_api_read'}`. A tight limit scoped to the tool an iterating agent calls most caps it without constraining the rest. For more information, see [apply-policies-to-tool-invocations.md](../configure-your-mcp/apply-policies-to-tool-invocations.md "mention").
{% endhint %}
...
1. As the same caller, call `stripe_api_read` six times within 60 seconds.
...
3. Using the upstream credential, read a customer record directly from Stripe.
...
A tool call through the Gateway has two legs. The **entrypoint** leg is the agent's call to the Composite MCP Server, and the **endpoint** leg is the Gateway's call to Stripe. Both legs together turn a tool call into an auditable event, because each leg answers a different question:
...
{% hint style="warning" %}
Stripe's MCP server returns JSON, and on that path response policies run before either leg is recorded. Both logged legs therefore show the same redacted payload, and you cannot prove redaction by comparing them. Confirm redaction by reading the record directly from the upstream with its own credential and comparing that against a read through the Composite MCP Server.

A streaming upstream records the endpoint leg differently. The Atlassian MCP server, measured on the same build and the same reporter settings, logged the endpoint leg before response policies ran. The personal data reached the log store in clear even though the agent received `[REDACTED]`. Two upstreams are not a rule, so confirm which way yours behaves with one permitted call. Treat payload logging as its own decision rather than as a consequence of redaction wherever the payload is sensitive.
{% endhint %}
...
For the OpenTelemetry span view, see [inspect-your-agent-log.md](../../observe/inspect-your-agent-log.md "mention").

#### Verification

To confirm that Stripe MCP interactions are recorded, complete the following steps:

1. As a permitted caller, call `stripe_api_read` on a customer record containing an email address.
...
3. On the **Response** page of the **Details** section, confirm that both response legs show `[REDACTED]`, which is the JSON-path behavior described in the preceding warning.
4. As a caller that the policy forbids, call `create_refund`.
...
2. Prompt the agent to retrieve a customer and to summarize their recent charges.
...
* **Compose per role rather than per upstream.** An approver role and a support role are better served by two servers than by one server and a forbid statement. Curation removes `create_refund` from `tools/list` as well as from the permitted set.
...
* [govern-mcp-tool-access.md](../configure-your-mcp/govern-mcp-tool-access.md "mention")
* [add-policies-to-mcp-server.md](../configure-your-mcp/add-policies-to-mcp-server.md "mention")
* [apply-policies-to-tool-invocations.md](../configure-your-mcp/apply-policies-to-tool-invocations.md "mention")
* [inspect-your-agent-log.md](../../observe/inspect-your-agent-log.md "mention")
* [monitor-your-mcp-servers.md](../../observe/monitor-your-mcp-servers.md "mention")
