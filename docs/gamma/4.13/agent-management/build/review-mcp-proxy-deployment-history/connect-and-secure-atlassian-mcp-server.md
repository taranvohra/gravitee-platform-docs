---
description: >-
  Connect Atlassian's MCP server to Gravitee, curate the tools it exposes, and
  authorize every call per caller identity. Follow the steps to get started.
---
...
1. [Connect the Atlassian MCP server](connect-and-secure-atlassian-mcp-server.md#connect-the-atlassian-mcp-server)
2. [Expose the Atlassian tools as a Composite MCP Server](connect-and-secure-atlassian-mcp-server.md#expose-the-atlassian-tools-as-a-composite-mcp-server)
3. [Authenticate access to your deployed Atlassian MCP server](connect-and-secure-atlassian-mcp-server.md#authenticate-access-to-your-deployed-atlassian-mcp-server)
4. [Restrict Atlassian tool access with fine-grained authorization](connect-and-secure-atlassian-mcp-server.md#restrict-atlassian-tool-access-with-fine-grained-authorization)
5. [Apply policies to the Atlassian MCP server](connect-and-secure-atlassian-mcp-server.md#apply-policies-to-the-atlassian-mcp-server)
6. [Observe Atlassian MCP interactions](connect-and-secure-atlassian-mcp-server.md#observe-atlassian-mcp-interactions)
...
{% hint style="info" %}
If you authenticated your Atlassian MCP server during the [Expose the Atlassian tools as a Composite MCP Server](connect-and-secure-atlassian-mcp-server.md#expose-the-atlassian-tools-as-a-composite-mcp-server) section, you can skip this section and go to [Restrict Atlassian tool access with fine-grained authorization](connect-and-secure-atlassian-mcp-server.md#restrict-atlassian-tool-access-with-fine-grained-authorization).
{% endhint %}
...
Every entity reference is an identifier, not a display name. `principal` takes the group's identifier, and `resource` takes the server-qualified tool identifier. For more information about layered governance for MCP tools, see [govern-mcp-tool-access.md](../configure-your-mcp/govern-mcp-tool-access.md "mention").
{% endhint %}
...
{% hint style="info" %}
To limit one tool rather than the whole server, add a **Condition** to the flow that matches the tool name, for example `{#context.attributes['mcp_tool_name'] == 'searchJiraIssuesUsingJql'}`. A tight limit scoped to the tool an iterating agent calls most caps it without constraining the rest. For more information, see [apply-policies-to-tool-invocations.md](../configure-your-mcp/apply-policies-to-tool-invocations.md "mention").
{% endhint %}
