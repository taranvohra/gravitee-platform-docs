---
description: >-
  Configure Claude Code to reach Anthropic through an LLM Proxy while users keep
  their own OAuth login. Follow the steps to set up the plan and headers.
---
...
The LLM Proxy provider must be configured with **Authentication: None**. Do not configure Bearer authentication or an Expression Language expression to copy the `Authorization` header. With no provider authentication configured, the request's existing `Authorization: Bearer <Claude OAuth token>` header passes through to Anthropic unchanged.

## Prerequisites

* Access to the Gamma console. For more information, see [Self-hosted installation guides](../../platform-management/install/self-hosted-installation-guides/) and [Hybrid installation guides](../../platform-management/install/hybrid-installation-guides/).
...
1. [Configure the LLM Proxy](connect-claude-code-through-an-llm-proxy.md#configure-the-llm-proxy)
2. [Configure model governance](connect-claude-code-through-an-llm-proxy.md#configure-model-governance)
3. [Create a plan and subscription](connect-claude-code-through-an-llm-proxy.md#create-a-plan-and-subscription)
4. [Configure Claude Code](connect-claude-code-through-an-llm-proxy.md#configure-claude-code)
...
* Remove any Bearer or API key authentication configured on the provider.
