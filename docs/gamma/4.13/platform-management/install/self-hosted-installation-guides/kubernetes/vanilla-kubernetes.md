---
description: >-
  Install a fully self-hosted Gamma platform on a vanilla Kubernetes cluster.
  Follow the steps to deploy every component with the Helm chart.
---
...
* **(Enterprise Edition only)** To enable Agent Management, you need an enterprise license that includes the `agent-management` pack. To get one, contact your Technical Account Manager. For more information, see [Add your license key](vanilla-kubernetes.md#enterprise-edition-only-add-your-license-key).
...
1. [#create-the-namespace](vanilla-kubernetes.md#create-the-namespace "mention")
2. [#deploy-mongodb-and-elasticsearch](vanilla-kubernetes.md#deploy-mongodb-and-elasticsearch "mention")
3. [#install-the-ingress-controller](vanilla-kubernetes.md#install-the-ingress-controller "mention")
4. [#configure-dns-resolution](vanilla-kubernetes.md#configure-dns-resolution "mention")
5. [#enterprise-edition-only-add-your-license-key](vanilla-kubernetes.md#enterprise-edition-only-add-your-license-key "mention")
6. [#prepare-the-values-yaml-for-helm](vanilla-kubernetes.md#prepare-the-values-yaml-for-helm "mention")
...
2.  Install Elasticsearch using the following commands. For more information, see the [official chart documentation](https://artifacthub.io/packages/helm/elastic/elasticsearch).
    ...
    ```
3.  Retrieve the `elastic` user password using the following command. You add it to your `values.yaml` in [Prepare the values.yaml for Helm](vanilla-kubernetes.md#prepare-the-values-yaml-for-helm):
...
3.  In your `values.yaml`, uncomment the license lines under the `api` service so the chart mounts the secret:
    ...
    \
    The [Install with Helm](vanilla-kubernetes.md#install-with-helm) step then mounts the secret automatically.

### Prepare the `values.yaml` for Helm

The Helm values consolidate every console and the API on `gamma.localhost`, point the platform at the datastores you deployed in [Deploy MongoDB and Elasticsearch](vanilla-kubernetes.md#deploy-mongodb-and-elasticsearch), and turn on Gamma.
...
1.  Create a `values.yaml` file in your working directory, and then copy the following configuration into it:
    ...
    # (see Deploy MongoDB and Elasticsearch).
    ```
2. Replace `[ELASTIC PASSWORD FROM ES INSTALLATION]` with the password you retrieved in [Deploy MongoDB and Elasticsearch](vanilla-kubernetes.md#deploy-mongodb-and-elasticsearch).
...
2. [#validate-the-endpoints](vanilla-kubernetes.md#validate-the-endpoints "mention")
3. [#validate-the-gateway](vanilla-kubernetes.md#validate-the-gateway "mention")
