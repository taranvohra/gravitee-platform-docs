...
{% hint style="info" %}
To learn more about Gravitee [Enterprise Edition](../../../readme/enterprise-edition/) and what's included in various enterprise packages, please:
...
{% hint style="info" %}
The examples in this section show `gravitee.yml` syntax. For APIM Helm chart deployments, the same blocks must be nested under `api:` or `gateway:` in your `values.yaml` file, depending on which component reads the secret. See [Helm Charts specifics](configuration.md#helm-charts-specifics) below for the equivalent Helm chart configuration.
{% endhint %}
