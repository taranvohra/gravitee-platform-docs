...
The following table summarizes reuse eligibility by key state:
...
{% hint style="warning" %}
**Security Consideration**: Always assign unique key values. Reusing an API key value across multiple clients or environments increases security risk because exposing that key compromises every endpoint that uses it.
{% endhint %}
...
7.  Observe that the revoked key from the closed subscription is displayed separately in the API keys table.<br>
...
* If no existing key with that value is found for the same API or API Product reference, a new key is created.
* If an existing key is found:
  ...
  * If custom API key reuse is enabled:
    * If the existing key is revoked or expired, the system reactivates and reuses it.
...
1. The existing API key record is reactivated: `revoked` is set to `false` and `revokedAt` is set to `null`.
...
5. No duplicate record is created for the same reference and key combination.
...
* Reuse is only permitted when the existing key belongs to the same application and the same API or API Product reference.
