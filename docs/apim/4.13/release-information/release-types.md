...
Gravitee currently has three types of releases: [Tech Preview](release-types.md#tech-preview-1), [Beta](release-types.md#beta), and [general availability](release-types.md#general-availability). Feature identification and usage considerations for each release type are summarized below:
...
When a feature or release version reaches general availability, it signifies that the software has been publicly released and is fully supported according to [Gravitee's support and maintenance policy](support-model/). Generally available features are officially documented and their interfaces are stable.
