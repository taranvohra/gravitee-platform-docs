...
* Users must have the **API\_PRODUCT\_DEFINITION:UPDATE** permission to assign sharding tags to API Products and to deploy API Products.
* Users must have the **API\_PRODUCT\_PLAN:CREATE** permission to assign sharding tags when creating a plan, or the **API\_PRODUCT\_PLAN:UPDATE** permission to assign sharding tags when editing a plan.
* Verifying deployment (license and compatibility check) requires the **API\_PRODUCT\_DEFINITION:READ** permission.
