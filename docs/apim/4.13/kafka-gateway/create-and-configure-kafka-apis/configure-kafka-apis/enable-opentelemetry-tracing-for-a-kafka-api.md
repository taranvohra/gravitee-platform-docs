...
This page explains how to enable OpenTelemetry tracing for a single Kafka native API once gateway-level tracing is in place. For the broader OpenTelemetry feature overview and the full attribute reference, see [OpenTelemetry](../../../analyze-and-monitor-apis/opentelemetry/).
...
6.  If deep debugging is required, enable the **Verbose** toggle to add per-phase, per-flow, and per-policy spans.
    ...
    <div data-gb-custom-block data-tag="hint" data-style="warning" class="hint hint-warning"><p>Verbose mode significantly increases trace volume on high-throughput APIs. Enable only for deep debugging.</p></div>
