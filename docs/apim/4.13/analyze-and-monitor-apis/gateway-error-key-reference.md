...
A backend that closes _cleanly_, on a response with no announced length, produces **no error key at all**. Closing is a legitimate way to delimit the body in that case.
