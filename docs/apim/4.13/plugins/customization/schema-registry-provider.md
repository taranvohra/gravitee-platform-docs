...
The contract lives in the `io.gravitee.resource:gravitee-resource-schema-registry-provider-api` Maven artifact. The [Confluent Schema Registry resource](../../create-and-configure-apis/apply-policies/resources/#confluent-schema-registry), an Enterprise Edition capability, implements the full `1.1.0` contract. For the common plugin scaffolding that applies to every plugin type, see [Customization](./).
...
1. Package the provider as a resource plugin and deploy it to the Gateway, following the structure described in [Customization](./).
2. Create the resource on an API, as described in [Resources](../../create-and-configure-apis/apply-policies/resources/).
