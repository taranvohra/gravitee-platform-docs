...
* When `API Product Primary Mode` is `GROUP`, you must belong to at least one group with a PRIMARY\_OWNER member for API\_PRODUCT scope. The target group for ownership transfer must have at least one member with PRIMARY\_OWNER role for API\_PRODUCT scope.
...
4.  Enter the name of the person or people that you want to add to the API Product Group, and then click **Select**.
...
5.  Navigate to the user you added, and then, from the **Role** dropdown menu, select a role from the available roles.
...
4. Select the transfer mode based on the primary owner mode:
   *   If the primary owner mode is `HYBRID`, select **API Product member** (choose from existing non-PRIMARY\_OWNER members), **Other user** (search for an organization user), or **Primary owner group** (select a group with a PRIMARY\_OWNER member).
...
5.  Select the new role for the current primary owner from the dropdown. PRIMARY\_OWNER is excluded. The first default role is selected by default.
