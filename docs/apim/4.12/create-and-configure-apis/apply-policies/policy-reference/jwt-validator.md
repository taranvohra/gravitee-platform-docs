...
Some authorization servers use OAuth2 protocol to provide access tokens. These access tokens can be in JWS/JWT format. For the RFC standards, see:
...
The JWT policy supports dot-notation for extracting values from nested claims in JWT tokens. Administrators can configure user and client ID extraction from hierarchical claim structures (e.g., `realm_access.preferred_username` or `act.repository`) without custom processing.

The JWT policy resolves claim paths using a two-tier lookup strategy. When a claim name contains a dot (e.g., `act.repository`), the policy first checks for a top-level claim with that exact literal name. If no flat claim exists, the policy splits the path on dots and traverses nested Map values. This preserves backward compatibility while enabling hierarchical claim access.

| Resolution Priority | Behavior                                      | Example                                            |
| ------------------- | --------------------------------------------- | -------------------------------------------------- |
| 1. Flat lookup      | Top-level claim with literal dot in name      | `"x.y": "flat-value"` → resolves to `"flat-value"` |
| 2. Nested traversal | Split on `.` and walk nested Map values       | `act.repository` → resolves `act` → `repository`   |
| 3. Null return      | Missing segment or non-Map intermediate value | Returns `null`                                     |

**Restrictions:**

* Array indexing is not supported (e.g., `claims[0].name` will not resolve)
* Empty string segments in paths (e.g., `a..b`) are treated as literal keys, not skipped
* Intermediate values must be Map objects; non-Map values return `null`
* Null or empty claim names return `null`
...
| ---------------------------------------------------------------------------------- | -------------------------------------- | :-------: | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <p>Client ID claim<br><code>clientIdClaim</code></p>                               | string                                 |           |             | Claim where the client ID can be extracted. Supports dot-notation for nested claims. A flat claim whose name literally contains a dot takes precedence. Configuring this field will override the standard behavior.     |
...
| <p>User claim<br><code>userClaim</code></p>                                        | string                                 |           | `sub`       | Claim where the user can be extracted. Supports dot-notation for nested claims. A flat claim whose name literally contains a dot takes precedence.                                                                      |
...
| <p>Extract client certificate from headers<br><code>extractCertificateFromHeader</code></p> | boolean                                |           |                   | Enabled to extract the client certificate from request header. Necessary when the mTLS connection is handled by a proxy.             |
...
Existing JWT policy configurations using flat claim names (e.g., `sub`, `client_id`) continue to work unchanged. No configuration updates are required. To adopt nested claim extraction, update the **User Claim** or **Client Id Claim** properties with dot-notation paths. For example, a JWT payload containing the following structure can be configured with **Client Id Claim** set to `act.repository` (resolves to `my-org/my-repo`) and **User Claim** set to `realm_access.preferred_username` (resolves to `alice`):
