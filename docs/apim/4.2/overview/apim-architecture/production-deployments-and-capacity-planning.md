...
# Production Deployments and Capacity Planning
...
<img src="../../.gitbook/assets/file.excalidraw.svg" alt="Load balancer" class="gitbook-drawing">
...
The[ Gateway internal API](../../getting-started/configuration/the-gravitee-api-gateway/gateway-internal-api.md) and [Management API Internal API](../../getting-started/configuration/apim-management-api/internal-api-1.md) provide a set of RESTful endpoints that enable administrators to retrieve vital information about the node status, configuration, health, and monitoring data.

**Mock Policy for Active Health Checks**

Utilizing an API with a [Mock policy](../../reference/policy-reference/mock.md) enables administrators to perform active health checks on the Gravitee APIM Gateways. By configuring mock endpoints that simulate various scenarios, such as successful requests, timeouts, or errors, administrators can verify the gateway's responsiveness and behavior under different conditions.

**Prometheus Metrics**

[Integration with Prometheus](../../getting-started/configuration/the-gravitee-api-gateway/logging.md#expose-metrics-to-prometheus) allows administrators to expose and collect metrics related to Gravitee APIM Gateways, including Vert.x 4 metrics. By accessing the `/_node/metrics/prometheus` endpoint on the internal API, administrators can retrieve detailed metrics with customizable labels, enabling them to monitor system performance and identify trends over time.
...
To avoid generating excessive data and reducing Gateway capacity, refrain from [activating the advanced logs](../../getting-started/configuration/the-gravitee-api-gateway/logging.md#modify-logging-information) on all API requests and responses.
